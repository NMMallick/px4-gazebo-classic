...
void GpsPlugin::OnWorldUpdate(const common::UpdateInfo& /*_info*/)
{
  ...
  if (bytes_received >= 2) {
      ...
      if (spoof_flag)
      {
          spoof_level = (0x00ff * buffer);
      }
  }

  // update noise parameters if gps_noise_ is set
  if (spoof_flag) {
    noise_gps_pos_.X() = spoof_level* gps_xy_noise_density_ * sqrt(dt) * randn_(rand_);
    noise_gps_pos_.Y() = spoof_level*gps_xy_noise_density_ * sqrt(dt) * randn_(rand_);
    noise_gps_pos_.Z() = spoof_level*gps_z_noise_density_ * sqrt(dt) * randn_(rand_);
    noise_gps_vel_.X() = spoof_level*gps_vxy_noise_density_ * sqrt(dt) * randn_(rand_);
    noise_gps_vel_.Y() = spoof_level*gps_vxy_noise_density_ * sqrt(dt) * randn_(rand_);
    noise_gps_vel_.Z() = spoof_level*gps_vz_noise_density_ * sqrt(dt) * randn_(rand_);
    if (spoof_level != 0x00)
    {
